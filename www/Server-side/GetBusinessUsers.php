...
class BusinessUserAPI {
    ...
    function GeBusinessUsers() {
	
			$rw_app_id = $_GET["id"];
				
			$query = "SELECT `UserID`, members.username username,`LastRatingTime`, `RatingScore`, `LastGoingClickTime` 
						FROM `businessuser`, members 
						WHERE businessuser.UserID=members.id and  BusinessID = $rw_app_id
