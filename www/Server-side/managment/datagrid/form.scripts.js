<!--// version 1.0.1
/* TODO - isSet - Parse (' "" ...)  - pass dig + lett -> current not works */
/* usage: <script type="text/JavaScript" src="form.scripts.js"></script> */
/* <input type="submit" name="button" value="Submit" onClick="return onSubmitCheck(document.forms['sasha']);"> */
...
var rtypes="rs";              
var vtypes="nifatelpylv";       // + z-zipcode
...
function isText(parm){return isValid(parm,lwr + upr + digits3 + textchars);}
...
function onReqAlert(frm,ind){title_of_field=getFieldTitle(frm,ind);alert("The <" + title_of_field + "> is a required field!\nPlease, enter a valid " + title_of_field + ".");frm.elements[ind].focus();if(frm.elements[ind].type.substring(0,6) !="select")frm.elements[ind].select();return false;}
...
function onNotEqualAlert(frm,ind){type_of_field=getValidateField(frm, ind, "name");title_of_field=getFieldTitle(frm,ind);if(type_of_field==0) type_of_field="required field";alert("The <" + title_of_field + "> field must be match with " + type_of_field + "!\nPlease, renter.");frm.elements[ind].focus();if(frm.elements[ind].type.substring(0,6) !="select") frm.elements[ind].select();return false;}
...
function onSubmitCheck(frm){
    ...
    for(ind=0;ind<frm.elements.length;ind++){         
        ...
        if(isSubmitReqType(is_required) && isSubmitVarType(a_type) && (frm.elements[ind].style.display !="none")){
            ...
            if(is_required=='r'){ if(isEmpty(field_value)){ return onReqAlert(frm,ind);}}
